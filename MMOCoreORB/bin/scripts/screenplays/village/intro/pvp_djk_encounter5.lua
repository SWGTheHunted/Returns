...
PVPDJKEncounter5 = Encounter:new {
	-- Task properties
	taskName = "PVPDJKEncounter5",
...
function PVPDJKEncounter5:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPDJKEncounter5:onLoot(pLootedCreature, pLooter, nothing)
...
function PVPDJKEncounter5:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPDJKEncounter5.taskName, pKiller) then
...
--function PVPDJKEncounter5:onsithKilled(pMellichae, pKiller)
...
function PVPDJKEncounter5:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPDJKEncounter5:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPDJKEncounter5:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPDJKEncounter5:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPDJKEncounter5:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
