...
PVPLJKEncounter5 = Encounter:new {
	-- Task properties
	taskName = "PVPLJKEncounter5",
...
function PVPLJKEncounter5:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPLJKEncounter5:onLoot(pLootedCreature, pLooter, nothing)
...
function PVPLJKEncounter5:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPLJKEncounter5.taskName, pKiller) then
...
--function PVPLJKEncounter5:onsithKilled(pMellichae, pKiller)
...
function PVPLJKEncounter5:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPLJKEncounter5:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPLJKEncounter5:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPLJKEncounter5:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPLJKEncounter5:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
